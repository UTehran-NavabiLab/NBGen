...
SC_MODULE( testbench ) {
    ...
    std::array<sc_core::sc_signal<sc_dt::sc_logic>*, 30> signal_arr;
    ...
    void signaling(void){
        for (int i=0; i < pow(2, 3); i++){
            ...
            serIn.write(input_signalling(2, i));
            wait(SC_ZERO_TIME);
            std::cout << "--------- Apply New input vector: " << std::endl; 
            std::cout << "input signals:  clk, rst, serIn = " << clk.read() << rst.read() << serIn.read()  << std::endl; 
            wait(5, SC_NS);
            ready2update.notify();
            wait(SC_ZERO_TIME);
        }
    ...
    void run_idd_testing(void){
        idd_test.update_signal<sc_dt::sc_logic, 30>(signal_arr);
        idd_test.back_trace("serOutValid");
    ...
    void access_signals(void){
        ...
        signal_arr[2] = &(serIn);
        idd_test.add_to_port_list("serIn");
        signal_arr[3] = &(serOutValid);
        signal_arr[4] = &(cut->S0);
        signal_arr[5] = &(cut->S1);
        signal_arr[6] = &(cut->S2);
        signal_arr[7] = &(cut->S3);
        ...
        signal_arr[16] = &(cut->S12);
        signal_arr[17] = &(cut->S13);
        signal_arr[18] = &(cut->S14);
        signal_arr[19] = &(cut->S15);
        signal_arr[20] = &(cut->S16);
        signal_arr[21] = &(cut->S17);
        signal_arr[22] = &(cut->S18);
        signal_arr[23] = &(cut->S19);
        signal_arr[24] = &(cut->S20);
        signal_arr[25] = &(cut->S21);
        signal_arr[26] = &(cut->S22);
        signal_arr[27] = &(cut->ps_0);
        signal_arr[28] = &(cut->ps_1);
        signal_arr[29] = &(cut->ps_2);
    }
