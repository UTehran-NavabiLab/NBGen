    ...
    def cells_declaration(self):
        ...
        for p in range(3):
            if (p == 0):
                ...
                cell_instantiation += WHITE_SPACE + WHITE_SPACE + f'{golden_instatnce_name} = new fulladder("{self.module_name}_gold", accessRegistry);\n'
            elif(p == 2):
                cell_instantiation += WHITE_SPACE + WHITE_SPACE + f'{faulty_instatnce_name} = new fulladder("{self.module_name}_faulty", accessRegistry);\n'
            # port mapping 
